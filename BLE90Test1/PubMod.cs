...
{
    public static long SetBit(long data, byte pos, bool val)
    {
        if (pos < 0) return data;
        if (pos > 64) return data;
        if (val)
        {
            data = data | (1 << pos);
        }
    ...
    public static bool GetBit(long data, byte pos)
    {
        return (data & (1 << pos)) > 0;
    }

    public static byte SetBit(byte data, byte pos, bool val)
    {
        if (pos < 0) return data;
        if (pos > 8) return data;
        if (val)
        {
    ...
    public static bool GetBit(byte data, byte pos)
    {
        return (data & (1 << pos)) > 0;
    }
    ...
    public static void IncrementLoop(ref int Pos, int Count)
    {
        Pos++;
        if (Pos >= Count)
        {
...
0xB40BBE37,0xC30C8EA1,0x5A05DF1B,0x2D02EF8D};
    ...
    public static byte[] ASCIIToBytes(string tdata, ushort length)
    {
    ...
    public static string BytesToASCII(byte[] tdata)
    {
        return ASCIIEncoding.ASCII.GetString(tdata);
    }
